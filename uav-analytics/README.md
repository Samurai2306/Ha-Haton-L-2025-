...
- Переменные окружения (на стороне API):
  - `AI_API_BASE` — базовый URL OpenAI‑совместимого API (напр., `https://api.deepseek.com`).
  - `AI_API_KEY` — ключ доступа (не храните в репозитории).
  - `AI_MODEL` — модель (напр., `deepseek-chat`).
  - `AI_TIMEOUT` — таймаут запроса (сек, по умолчанию 20).
- Эндпоинт: `POST /ai/analyze` — тело запроса — фильтры `{date_from, date_to, region, zone, altitude_category}`. Возвращает краткий аналитический отчёт (LLM), при отсутствии ключа — эвристическое резюме.
- В веб‑клиенте (страница «Обзор») появится кнопка «Анализ ИИ».
...
**Геопривязка (регламент и шейпы)**
- Источник границ: официальные шейп‑файлы Росреестра (субъекты РФ). Рекомендуется конвертировать в GeoJSON и хранить в каталоге `data/shapes/`.
- Обновление: не реже 1 раза в месяц. Релиз‑скрипт должен скачивать свежие шейпы, валидировать геометрию и обновлять справочник соответствий.
- Технический план внедрения:
  1) добавить GeoJSON субъектов РФ и справочник кодов;
  2) реализовать Point‑in‑Polygon (DEP/ARR) → `region_resolved` (через `shapely`/`geopandas` или быстрый R‑tree);
  3) переиспользовать `region_resolved` для карт (хлороплет) и агрегатов;
  4) добавить `make shapes-update` и раздел в CI.
